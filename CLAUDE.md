...
13. **Schema text is data, wherever it is written.** A version's descriptions can come from an
    ...
    writes every line inside one block, on lines of its own, since the compiler attaches nothing
    to a comment that shares a line with the token before it. A tag the contract carries is one
    the generator wrote from what a schema declares. Escaping an `@` rather than replacing it is
    not enough: `stripInternal` reads the comment text and would remove a declaration while what
    refers to it stays. Names
    and values reach generated code through `JSON.stringify` or `assertIdentifier`, never by
    interpolation. Characters that do not display are refused when a version is read, in names
