...
describe("typeExpression", () => {
  ...
  it("refuses a composition with more ways through than it writes out", () => {
    // Dropping the branches would be sound, and would leave a contract that compiles while
    // admitting requests the gateway rejects. The schema is what wants looking at, so it is
    // refused. Six nested pairs inside one branch is 64 ways through.
    ...
    ).toThrow(/more than 24 ways through/);

    // Five pairs is 32, still refused; four is 16 and written out in full.
    const ways = (count: number) =>
      type({
        type: "object",
        allOf: Array.from({ length: count }, (_, index) => pair(String(index))),
      });

    expect(() => ways(5)).toThrow(/more than 24 ways through/);
    expect(() => ways(4)).not.toThrow();
  });
  ...
  it("counts a union's branches as the nesting they are", () => {
    ...
    // A union written beside another is not nesting: both are branches of the same schema. Thirty
    // of them run out of combinations long before the depth bound, which is the point: what they
    // exhaust is the budget for writing the branches out, not the reader's descent.
    ...
    expect(() => type(wide)).toThrow(/more than 24 ways through/);
  });
