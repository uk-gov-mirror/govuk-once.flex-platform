...
// How many ways through a composition are worth writing out. Nothing in these gateways' schemas
// comes close; a composition that does exceed it is emitted without its unions instead.
...
function expand(
  ...
  for (const branch of group.branches) {
    const combinations = expand(
      withBranch(base, branch, ctx, group.depth),
      ctx,
      depth,
      budget,
    );
    if (combinations === undefined) return undefined;
    expanded.push(...combinations);
  }
  return expanded;
}
...
function expressionOf(
  ...
  if (combinations === undefined) {
    // More ways through than are worth writing out. The unions are dropped rather than
    // intersected with what encloses them: a constraint this contract cannot express has to
    // widen the type, never narrow it, or a request the gateway accepts would not typecheck.
    return emitComposition({ ...composition, unions: [] }, ctx, depth);
  }
